...
Parameters:
out_count: A pointer to an integer where the number of collected wsis will be stored.
...
Parses the command-line arguments to get the database file name.
Initializes the database by calling init_db().
Sets up the libwebsockets context and protocol.
Starts the libwebsockets event loop.
Cleans up by calling lws_context_destroy() and close_db() when the server exits.
